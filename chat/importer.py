...
def import_chatgpt_json(json_contents: str, extracted_files_base_path: str, task_id=None, task_statuses=None):
    ...
    for item_id, item in mapping.items():
        if not isinstance(item, dict):
            # print(f"Skipping invalid mapping item for chat {chat_id}: {item}") # Removed verbose logging
            continue
                    ...
                    if asset_pointer:
                        ...
                        if match:
                            ...
                            for root_dir, _, files in os.walk(extracted_files_base_path):
                                for fname in files:
                                    # Look for filenames that contain the asset_uuid, typically in formats like
                                    # file-<UUID>-name.ext or file_<UUID>-name.ext
                                    ...
                                    if (asset_uuid in fname) and \
                                       fname.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg')):
                                        ...
                                        # Use original text if present, otherwise a placeholder
                            ...
                            if not image_found:
                                ...
                                # print(f"Warning: Image file for asset {asset_uuid} not found in {extracted_files_base_path}") # Removed verbose logging
                        else:
    ...
    if not processed_messages_data:
        # If no processable messages, create a session with 0 messages
